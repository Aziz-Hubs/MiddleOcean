...
export default function ContactUs1({ settings }: ContactUs1Props) {
  ...
  const displayPhone = settings?.phone || t("phone");
  const displayEmail = settings?.email || t("email");
  const displayAddress = settings?.address?.[locale as "en" | "ar"] || t("address");
  const mapLat = settings?.mapCoordinates?.lat || 24.705140;
                ...
                <div className={cn("grid grid-cols-1 sm:grid-cols-2 gap-8 mb-10 w-full", isRtl ? "text-right" : "text-left")}>
                  ...
                  <div className={cn("flex items-start gap-4", isRtl ? "flex-row-reverse" : "flex-row")}>
                    ...
                    <div className={isRtl ? "text-right" : "text-left"}>
                      ...
                      <p dir="ltr" className="text-foreground/80 text-sm mt-1.5 font-sans">
                        {displayPhone}
                      </p>
                    </div>
                  </div>
